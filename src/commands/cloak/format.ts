...
export function formatProfile(p: CbmProfile, fingerprintMode = false): string {
  ...
  if (fingerprintMode) {
    ...
    lines.push(`  device-memory:    ${p.device_memory ?? '—'} GB       hw-concurrency: ${p.hardware_concurrency ?? '—'}`);
    ...
    if (p.noise_enabled !== undefined) {
      lines.push(`  noise:            ${p.noise_enabled ? 'enabled (unique per seed)' : 'disabled (stable identity)'}`);
    }
    ...
    lines.push(`  clear-on-launch:  ${p.clear_on_launch ? 'yes' : 'no'}  storage-quota: ${p.storage_quota_mb ? `${p.storage_quota_mb}MB` : '—'}`);
    ...
    lines.push(`  humanize:         ${p.humanize ? `yes (${p.human_preset})` : 'no'}  headless: ${p.headless}`);
    lines.push(`  auto_launch:      ${p.auto_launch}  restart: ${p.restart_on_crash} (max ${p.max_restarts})`);
  } else {
    // ── Standard view ──
    lines.push(`  proxy:            ${proxy}`);
    lines.push(`  timezone/locale:  ${p.timezone ?? '—'} / ${p.locale ?? '—'}`);
    lines.push(`  screen:           ${p.screen_width}x${p.screen_height}`);
    lines.push(`  gpu:              ${p.gpu_vendor ?? '—'} / ${truncate(p.gpu_renderer ?? '—', 50)}`);
    lines.push(`  hw-concurrency:   ${p.hardware_concurrency ?? '—'}  device-memory: ${p.device_memory ? `${p.device_memory}GB` : '—'}`);
    lines.push(`  brand:            ${p.brand ?? 'auto'} v${p.brand_version ?? 'auto'}`);
    lines.push(`  webrtc-ip:        ${p.webrtc_ip ?? (proxy !== '—' ? 'auto' : '—')}  noise: ${p.noise_enabled ? 'on' : 'off'}`);
    lines.push(`  humanize:         ${p.humanize ? `yes (${p.human_preset})` : 'no'}  geoip: ${p.geoip}  headless: ${p.headless}`);
    lines.push(`  clear-on-launch:  ${p.clear_on_launch ? 'yes' : 'no'}  scale-factor: ${p.device_scale_factor ?? '—'}`);
    lines.push(`  auto_launch:      ${p.auto_launch}  restart_on_crash: ${p.restart_on_crash} (max ${p.max_restarts})`);
