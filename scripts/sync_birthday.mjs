...
async function doReq(label, email, markerBytes, opts={}) {
  const caller=[...fv(2,1)];
  let gu=[...fm(1,caller),...fv(2,1),...markerBytes,...fv(8,1),...fv(4,12)];
  if(opts.birthday) gu.push(...fb(3, opts.birthday)); // store_birthday=3 in ClientToServerMessage
  const reqBytes=Buffer.from([...fb(1,email),...fv(2,99),...fv(3,2),...fm(5,gu)]);
  ...
  if(!val)return{label,status:'no-result'};
  const p=JSON.parse(val);
  if(p.e)return{label,status:'err',error:p.e};
  const raw=Buffer.from(p.b);
  if(p.s!==200)return{label,status:p.s,bodyLen:p.l};
  const top=parsePb(raw).fields;
  const guBytes=top.find(f=>f[0]===2)?.[2];
  const guf=guBytes?parsePb(guBytes).fields:[];
  const entries=guf.filter(f=>f[0]===1).length;
  const mkrs=guf.filter(f=>f[0]===5).length;
  const keys=guf.filter(f=>f[0]===6);
  const birthday=top.find(f=>f[0]===6)?.[2]?.toString('utf8');
  return{label,status:200,entries,mkrs,keys:keys.length,keyLens:keys.map(k=>k[2].length),birthday,bodyLen:p.l,raw};
}

// Step 1: no-marker → get birthday + encryption key
const r1=await doReq('step1-no-marker', acct.email, []);
console.log('step1:', r1.status, 'keys:', r1.keys, 'birthday:', r1.birthday?.slice(0,40));

// Step 2: nigori marker (no token) + birthday
const nigoriMarker=[...fm(3,[...fv(1,47745)])];
const r2=await doReq('step2-nigori+bday', acct.email, nigoriMarker, {birthday:r1.birthday});
console.log('step2:', r2.status, 'entries:', r2.entries, 'markers:', r2.mkrs);

// Step 3: nigori marker (empty token) + birthday
const nigoriEmpty=[...fm(3,[...fv(1,47745),...fb(2,Buffer.alloc(0))])];
const r3=await doReq('step3-nigori-empty+bday', acct.email, nigoriEmpty, {birthday:r1.birthday});
console.log('step3:', r3.status, 'entries:', r3.entries, 'markers:', r3.mkrs);

// Step 4: bookmarks marker (no token) + birthday
const bmNoTok=[...fm(3,[...fv(1,32904)])];
const r4=await doReq('step4-bookmark+bday', acct.email, bmNoTok, {birthday:r1.birthday});
console.log('step4:', r4.status, 'entries:', r4.entries, 'markers:', r4.mkrs);

// Step 5: passwords marker (no token) + birthday
const pwNoTok=[...fm(3,[...fv(1,45873)])];
const r5=await doReq('step5-password+bday', acct.email, pwNoTok, {birthday:r1.birthday});
console.log('step5:', r5.status, 'entries:', r5.entries, 'markers:', r5.mkrs);
