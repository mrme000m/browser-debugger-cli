#!/usr/bin/env node
/**
 * Alpha.Net.BD client portal (portal.alpha.net.bd/clientarea/) - WHMCS login.
 *
 ...
 *
 * Usage:
 *   node alpha_login.mjs "user:pass" ...
 *   node alpha_login.mjs --batch <file> [--gap 12] [--out /tmp/alpha_batch_results.json]
 *
...
const URL = 'https://portal.alpha.net.bd/clientarea/';
...
async function tryLogin(u, p) {
  ...
  try {
    ...
    try {
      ...
      await page.click('form button[type=submit]', { timeout: 20000, force: true });
...
if (!pairs.length) { console.log('usage: node alpha_login.mjs "user:pass" | --batch <file>'); process.exit(1); }

const gapIdx = args.indexOf('--gap');
const GAP = gapIdx !== -1 ? Number(args[gapIdx + 1]) || 12 : 12;
const outIdx = args.indexOf('--out');
const OUT = outIdx !== -1 ? args[outIdx + 1] : '/tmp/alpha_batch_results.json';
