...
const acct=ALL[0]; // ft-vahan
console.log('acct:', acct.email);
const at=await mintToken(acct.refresh_token);

async function probe(name, ids, addBday=false, addToken=false) {
  const caller=[...fv(2,1)];
  let markers=ids.map(id=>fm(3, addToken ? [...fv(1,id),...fb(2,Buffer.alloc(0))] : [...fv(1,id)]));
...
await probe('1-marker-bm-no-tok', [32904]);
await probe('1-marker-bm-empty-tok', [32904], false, true);
await probe('1-marker-nigori-no-tok', [47745]);
await probe('2-marker-bm+pw-no-tok', [32904,45873]);
